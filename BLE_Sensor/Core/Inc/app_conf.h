 ...
 */
#define CFG_BLE_NUM_GATT_SERVICES   (9)
 ...
 */
#define CFG_BLE_NUM_GATT_ATTRIBUTES (80)
...
/* STOP mode is enabled: the low-power framework may enter STOP1 in idle
 * periods; the B2 button (EXTI/wakeup) brings the system back.
 * Note: this BLE_Sensor baseline only implements the STOP1 entry path in
 * stm32_lpm_if.c, so STOP2 remains disabled (no matching implementation). */
...
typedef enum
{
  ...
  CFG_TASK_MOTION_UPDATE_ID,
  CFG_TASK_ENV_UPDATE_ID,
  CFG_TASK_LED_ID,
