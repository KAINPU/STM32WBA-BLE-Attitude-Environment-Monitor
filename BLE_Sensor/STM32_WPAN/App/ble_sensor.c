...
/* USER CODE BEGIN Context */
  ...
  uint16_t  TelemetrySvcHdle;               /**< Project telemetry service handle */
  uint16_t  Telemetry_CCharHdle;            /**< Project telemetry characteristic handle */
...
/* USER CODE BEGIN PF */
#define COPY_TELEMETRY_SERVICE_UUID(uuid_struct) \
  COPY_UUID_128(uuid_struct,0x7a,0x11,0x2d,0x90,0x42,0x5b,0x4e,0x83,0x92,0x9b,0x1d,0x6e,0x7a,0x20,0x00,0x00)
#define COPY_TELEMETRY_C_UUID(uuid_struct)  \
  COPY_UUID_128(uuid_struct,0x7a,0x11,0x2d,0x90,0x42,0x5b,0x4e,0x83,0x92,0x9b,0x1d,0x6e,0x7a,0x20,0x00,0x01)
...
void BLE_SENSOR_Init(void)
{
  ...
  {
    tBleStatus tel_ret;
    uint8_t  tel_max_attr = 4;

    COPY_TELEMETRY_SERVICE_UUID(uuid.Char_UUID_128);
    tel_ret = aci_gatt_add_service(UUID_TYPE_128,
                                   (Service_UUID_t *)&uuid,
                                   PRIMARY_SERVICE,
                                   tel_max_attr,
                                   &(BLE_SENSOR_Context.TelemetrySvcHdle));
    if (tel_ret != BLE_STATUS_SUCCESS)
    {
      LOG_INFO_BLE("  Fail   : aci_gatt_add_service command: TELEMETRY, error code: 0x%02X\n", tel_ret);
    }
    else
    {
      COPY_TELEMETRY_C_UUID(uuid.Char_UUID_128);
      tel_ret = aci_gatt_add_char(BLE_SENSOR_Context.TelemetrySvcHdle,
                                  UUID_TYPE_128,
                                  (Char_UUID_t *)&uuid,
                                  SizeTelemetry_C,
                                  CHAR_PROP_NOTIFY,
                                  ...
                                  0x10,
                                  CHAR_VALUE_LEN_CONSTANT,
                                  &(BLE_SENSOR_Context.Telemetry_CCharHdle));
      if (tel_ret != BLE_STATUS_SUCCESS)
      {
        LOG_INFO_BLE("  Fail   : aci_gatt_add_char command: TELEMETRY_C, error code: 0x%02X\n", tel_ret);
      }
      else
      {
        LOG_INFO_BLE("  Success: aci_gatt_add_char command: Telemetry_CCharHdle = 0x%04X\n",
                     BLE_SENSOR_Context.Telemetry_CCharHdle);
      }
    }
  }
...
tBleStatus BLE_SENSOR_UpdateValue(BLE_SENSOR_CharOpcode_t CharOpcode, BLE_SENSOR_Data_t *pData)
{
  ...
  switch(CharOpcode)
  {
    ...
    default:
      /* USER CODE BEGIN Service1_App_Update_Char_telemetry */
      if (CharOpcode == BLE_SENSOR_TELEMETRY_C)
      {
        ret = aci_gatt_update_char_value(BLE_SENSOR_Context.TelemetrySvcHdle,
                                         BLE_SENSOR_Context.Telemetry_CCharHdle,
                                         0, /* charValOffset */
                                         pData->Length,          /* charValLen */
                                         (uint8_t *)pData->p_Payload);
        if (ret != BLE_STATUS_SUCCESS)
        {
          LOG_INFO_BLE("  Fail   : aci_gatt_update_char_value TELEMETRY_C command, error code: 0x%02X\n", ret);
        }
      }
      /* USER CODE END Service1_App_Update_Char_telemetry */
      break;
  }
