...
#define BLE_HOST_NVM_SIZE_U64          ((SNVMA_NVM_ID_1_BUFFER_1_SIZE / sizeof(uint64_t) ) - 2)
...
/* Dynamic advertising interval (ms), set by APP_BLE_SetAdvInterval() */
static uint16_t adv_interval_min_ms = ADV_INTERVAL_MIN;
static uint16_t adv_interval_max_ms = ADV_INTERVAL_MAX;
BLE_SENSOR_APP_ConnHandleNotEvt_t BLE_SENSORHandleNotification;

static char a_GapDeviceName[] = {  'S', 'e', 'n', 's', 'o', 'r' }; /* Gap Device Name */

/* Advertising Data */
uint8_t a_AdvData[26] =
{
  9, AD_TYPE_COMPLETE_LOCAL_NAME, 'W', 'B', 'A', '5', 'M', '_', 'X', 'X',  /* Complete name */
...
PLACE_IN_SECTION("TAG_HostStack") static uint64_t host_nvm_buffer[BLE_HOST_NVM_SIZE_U64];
...
void APP_BLE_Init(void)
{
  ...
  uint8_t status = SNVMA_Register(APP_BLE_Host_SNVMA_BufferId_1_1,
                  (uint32_t *)host_nvm_buffer,
                  (BLE_HOST_NVM_SIZE_U64 * sizeof(uint64_t)) / sizeof(uint32_t) );
  if (status != SNVMA_ERROR_OK)
  {
    LOG_ERROR_APP( "SNVMA registration for BLE Host stack failed with status: %d", status);
    Error_Handler();
  }
  ...
  /* Realize a restore */
  SNVMA_Restore(APP_BLE_Host_SNVMA_BufferId_1_1);
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void *p_Pckt)
{
  ...
  switch (p_event_pckt->evt)
  {
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE:
    {
      ...
      BLE_SENSORHandleNotification.EvtOpcode = BLE_SENSOR_DISCON_HANDLE_EVT;
      BLE_SENSORHandleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
      BLE_SENSOR_APP_EvtRx(&BLE_SENSORHandleNotification);
      BLE_SENSOR_APP_ConnectionState(0U);
    ...
    case HCI_LE_META_EVT_CODE:
    {
      ...
      switch (p_meta_evt->subevent)
      {
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE:
        {
          ...
          BLE_SENSORHandleNotification.EvtOpcode = BLE_SENSOR_CONN_HANDLE_EVT;
          BLE_SENSORHandleNotification.ConnectionHandle = p_enhanced_conn_complete->Connection_Handle;
          BLE_SENSOR_APP_EvtRx(&BLE_SENSORHandleNotification);
          BLE_SENSOR_APP_ConnectionState(1U);
...
void APP_BLE_Procedure_Gap_Peripheral(ProcGapPeripheralId_t ProcGapPeripheralId)
{
  ...
  switch(ProcGapPeripheralId)
  {
    ...
    {
      paramA = ADV_INT_MS(adv_interval_min_ms);
      paramB = ADV_INT_MS(adv_interval_max_ms);
...
void APP_BLE_HostNvmStore(void)
{
  ...
  SNVMA_Write(APP_BLE_Host_SNVMA_BufferId_1_1, BLE_NvmCallback);
}
...
static uint8_t HOST_BLE_Init(void)
{
  ...
  pInitParams.nvm_cache_max_size      = BLE_HOST_NVM_SIZE_U64;
  pInitParams.nvm_cache_size          = BLE_HOST_NVM_SIZE_U64 - 1;
...
/* Add number of record for Device Info Characteristic */
  ...
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : aci_hal_write_config_data command - CONFIG_DATA_PUBLIC_ADDRESS_OFFSET, result: 0x%02X\n", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: aci_hal_write_config_data command - CONFIG_DATA_PUBLIC_ADDRESS_OFFSET\n");
  ...
  */
  ...
  a_GATT_DevInfoData[4] = BOARD_ID_B_WBA5M_WPAN;
  ...
  a_GATT_DevInfoData[6] = WPAN_FW_MAJOR_VERSION;
  a_GATT_DevInfoData[7] = WPAN_FW_MINOR_VERSION;
  a_GATT_DevInfoData[8] = WPAN_FW_SUBVERSION;
  a_GATT_DevInfoData[9] = WPAN_FW_BRANCH;
  a_GATT_DevInfoData[10] = WPAN_FW_BUILD;
  ...
  a_GATT_DevInfoData[11] = FW_ID_SENSOR;
    ...
    case AD_TYPE_MANUFACTURER_SPECIFIC_DATA:
      {
        ...
        p_adv_data[i+5] = BOARD_ID_B_WBA5M_WPAN; /* Board ID */
        p_adv_data[i+6] = FW_ID_SENSOR; /* FW ID */
...
void APP_BSP_Button2Action(void)
{
  ...
  /* Long press (3s): enter deep sleep (disconnected only) */
  if (APP_BSP_ButtonIsLongPressed(B2))
  {
    BLE_SENSOR_APP_EnterDeepSleep();
    return;
  }

  BLE_SENSOR_APP_ButtonAction();
...
void APP_BLE_RequestConnectionUpdate(uint16_t min_ms, uint16_t max_ms)
{
  if (bleAppContext.connectionHandle == 0xFFFFU)
  {
    return;
  }
  /* Convert ms to 1.25 ms units */
  (void)aci_l2cap_connection_parameter_update_req(
      bleAppContext.connectionHandle,
      (uint16_t)(min_ms * 4U / 5U),
      (uint16_t)(max_ms * 4U / 5U),
      0U,   /* slave latency */
      500U  /* supervision timeout in units of 10 ms → 5 s */
  );
  LOG_INFO_APP("Conn param update requested: %u-%u ms\n", min_ms, max_ms);
}

void APP_BLE_SetAdvInterval(uint16_t min_ms, uint16_t max_ms)
{
  adv_interval_min_ms = min_ms;
  adv_interval_max_ms = max_ms;
  LOG_INFO_APP("Adv interval set: %u-%u ms\n", min_ms, max_ms);
}
